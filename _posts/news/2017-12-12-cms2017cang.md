...
(1) improving model performance under small data, and (2) measuring model interpretability. 
Our preliminary results lead to the following conclusions:

1. It is possible to improve model performance under small data, by incorporating physically meaningful constraints to the
model.
...
model interpretability, and are the current barrier for wider adoption of machine learning techniques for material design.
...
**We demonstrate the interpretability of a sandstone-Young's modulus model, and discuss how model interpretability can 
